...
Wait ... why should mount namespaces be “special”?

Well, mount point propagation especially when you least expect it. And the
“/sys/class/net” branch of [sysfs(5)], for example.
...
which can be easily verified, not least in the self-test units of this package –
the sysfs locks the “sys/class/net“ view to the network namespace of the
(OS-level) thread that mounted that particular sysfs instance.
...
network namespace, and then also mount a new sysfs instance onto “/sys” to get a
consistent view.
